        ...
        <div className="relative h-48 w-full bg-white/[0.02] border border-white/5 rounded-[2rem] overflow-hidden p-6 flex flex-wrap gap-4 items-center justify-center">
            {KEYWORDS.map((kw, i) => (
                <motion.div
                    ...
                    className="text-[13px] font-black uppercase tracking-widest cursor-default select-none"
                >
                    {kw.text}
                </motion.div>
            ))}
            ...
            <div className="absolute top-4 left-6 flex items-center gap-2">
                ...
                <span className="text-[13px] font-black text-slate-500 tracking-widest break-keep">많이 찾는 상징</span>
            </div>
        </div>
    );
}
