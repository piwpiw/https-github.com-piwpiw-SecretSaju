...
export default function SelectionPage() {
    ...
    const services = [
        {
            icon: Sun,
            title: locale === 'ko' ? '오늘의 운세' : 'Daily Fortune',
            subtitle: locale === 'ko' ? '매일 업데이트 되는 맞춤 운세' : 'Personalized Daily Report',
            description: locale === 'ko' ? '당신만을 위한 일일 애정운, 재물운, 그리고 프리미엄 딥 인사이트를 분석합니다.' : 'Scans your daily love, wealth, and provides premium deep insights.',
            badge: locale === 'ko' ? '매일 무료' : 'Free Daily',
            href: '/daily',
        ...
        {
            icon: Crown,
            title: locale === 'ko' ? '2026 신년운세' : '2026 Fortune',
            subtitle: locale === 'ko' ? '비범한 한 해의 예측' : 'Your Yearly Overview',
            description: locale === 'ko' ? '올해 당신이 맞이할 가장 거대한 운명의 파동과 조언을 확인하세요.' : 'Check the biggest waves of destiny and advice you will face this year.',
            badge: locale === 'ko' ? '기본 제공' : 'Included',
            href: '/fortune',
        ...
        {
            icon: Sparkles,
            title: locale === 'ko' ? '프리미엄 사주풀이' : 'Premium Saju Reading',
        ...
        {
            icon: Flame,
            title: locale === 'ko' ? '궁합 열람소' : 'Compatibility Matrix',
            subtitle: locale === 'ko' ? '우리의 싱크로율' : 'Our Synchro Rate',
            description: locale === 'ko' ? '두 운명의 얽힘을 계산하여 관계의 주도권과 주의점을 분석합니다.' : 'Analyzes power dynamics and cautions by calculating intertwined destinies.',
            badge: locale === 'ko' ? '젤리 2개' : '2 Jellies',
            href: '/compatibility',
        ...
        {
            icon: Rocket,
            title: locale === 'ko' ? '인연 대시보드' : 'Destiny Network',
            subtitle: locale === 'ko' ? '내 인맥의 중심' : 'My Network Hub',
            description: locale === 'ko' ? '저장된 모든 사람들의 Saju를 연결하여 당신만의 운명망을 구성합니다.' : 'Builds your own destiny web by connecting stored Saju profiles.',
            badge: locale === 'ko' ? '기본 무료' : 'Default Free',
            href: '/dashboard',
            ...
            <div className="max-w-5xl mx-auto px-6 py-16 relative z-10">
                ...
                <div className="text-center md:text-left mb-20 border-b border-border-color pb-16">
                    ...
                    <h1 className="text-5xl md:text-7xl font-black text-foreground italic tracking-tighter uppercase mb-4">
                        {locale === 'ko' ? '운명' : 'Destiny'} <span className="text-primary italic">{locale === 'ko' ? '서비스' : 'Services'}</span>
                    </h1>
                ...
                {profiles.length > 0 && (
                    <div className="mb-16">
                        <label className="text-xl font-bold flex items-center gap-4 text-secondary mb-6 pl-2">
                            <Star className="w-6 h-6 text-primary" /> {locale === 'ko' ? '서비스 대상 변경' : 'Target Profile'}
                        </label>
                        <div className="relative group max-w-md">
                            ...
                            >
                                <option value="" disabled>{locale === 'ko' ? '누구의 운세를 볼까요?' : 'Whose fortune to view?'}</option>
                                {profiles.map((p) => (
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mt-16 bg-surface rounded-5xl p-16 text-center border-2 border-dashed border-primary/30 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <h2 className="text-3xl md:text-4xl font-black text-foreground mb-4">
                                {locale === 'ko' ? '아직 등록된 사주 노드가 없습니다' : 'No Destiny Profile Yet'}
                            </h2>
                            <p className="text-xl text-secondary mb-10 font-medium">
                                {locale === 'ko' ? '분석을 시작하려면 당신의 프로필을 먼저 만들어주세요.' : 'Create your initial profile to start the analysis.'}
                            </p>
                            <Link href="/my-saju/add" className="inline-flex items-center gap-4 px-10 py-6 rounded-4xl bg-primary text-white font-black text-xl hover:scale-105 transition-all shadow-xl shadow-primary/20 uppercase tracking-widest">
                                {locale === 'ko' ? '플로필 등록하기' : 'Create Profile'} <ChevronRight className="w-6 h-6" />
                            </Link>
                        </div>
                    </motion.div>
                )}
