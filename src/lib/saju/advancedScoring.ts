 ...
 *
 * 봄(寅卯): 木旺, 火相, 水休, 金囚, 土死
 * 여름(巳午): 火旺, 土相, 木休, 水囚, 金死
 * 가을(申酉): 金旺, 水相, 土休, 火囚, 木死
 * 겨울(亥子): 水旺, 木相, 金休, 土囚, 火死
 * 사왕지절(辰戌丑未): 土旺, 金相, 火休, 木囚, 水死
 *
 * 辰戌丑未 는 계절 끝의 토왕절이다. 예전에는 이 넷을 그냥 봄/여름/가을/겨울
 * 행에 편입시켜서, 토 일간이 토월에 태어나도 득령 0~10점을 받았다. 그래서
 * 진월 무토 전왕 사주가 "신약"으로 나오고, 그 강약이 그대로 억부용신 선정에
 * 들어가 반대 방향 용신이 나갈 수 있었다.
 */
const WANGSEONG_SCORE: Record<string, Record<WuxingElement, number>> = {
    ...
    // 사왕지절 (土旺) — 토가 왕, 토가 낳는 금이 상, 토를 낳는 화가 휴,
    // 토를 극하는 목이 수, 토가 극하는 수가 사
    辰: { 土: 30, 金: 20, 火: 10, 木: 5, 水: 0 },
    戌: { 土: 30, 金: 20, 火: 10, 木: 5, 水: 0 },
    丑: { 土: 30, 金: 20, 火: 10, 木: 5, 水: 0 },
    未: { 土: 30, 金: 20, 火: 10, 木: 5, 水: 0 },
};
...
function calculateDeukse(
    ...
    // 일간을 뺀 천간은 3개뿐이므로, 40점 만점을 3등분해서 채운다.
    // 예전에는 1개당 10점이라 문서상 만점 40이 실제로는 30까지만 나왔고,
    // 총점 분포가 신약 쪽으로 쏠렸다.
    return Math.min(40, Math.round((helpCount * 40) / 3));
}
