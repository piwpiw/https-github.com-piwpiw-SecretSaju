...
const REPORT_DIR = path.join(TEMP_DIR, "reports", "ux-loop");
const DISPATCH_DIR = path.join(TEMP_DIR, "reports", "dispatch");
...
const SUMMARY_FILE = path.join(REPORT_DIR, "residual-issue-queue-summary.md");
const WORKLIST_FILE = path.join(REPORT_DIR, "continuous-development-queue.md");
const TASK_POOL_FILE = path.join(DISPATCH_DIR, "task-pool.jsonl");
const WORKER_TASK_POOL_FILE = path.join(DISPATCH_DIR, `task-pool-w${LOOP_WORKER_ID}.jsonl`);
...
async function appendHourlyReport({ batchIndex, from, to, cycles, payloads }) {
  ...
  await fs.mkdir(path.dirname(HOURLY_REPORT_FILE), { recursive: true });
...
async function upsertWorklist(payload) {
  if (!LOOP_WORKER_DOCS) return;
  await fs.mkdir(path.dirname(WORKLIST_FILE), { recursive: true });
