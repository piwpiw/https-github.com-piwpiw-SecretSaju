...
DB 내부 상세는 **`scripts/ops/verify-db.sql`** 를 Supabase SQL Editor 에 붙여 넣어라 (읽기 전용·안전, **단일 쿼리** — SQL Editor 가 마지막 결과만 보여주기 때문). 마이그레이션 001~010 중 무엇이 적용됐는지, RLS 가 켜져 있는지, 잔액 변경 트리거가 잘못 생기지 않았는지 표로 나온다. **"미적용"으로 나온 것만** 순서대로 실행하면 된다.
