...
- `scripts/mcp-browser-smoke.mjs` passed in a clean browser run at about 5.8s during direct benchmark on a healthy local server.
- Warm CDP reuse was roughly similar at about 5.7s; first CDP attach was slower.
- `scripts/smoke-admin-mode.mjs` was updated to inject admin bypass state at the context level so screen checks do not block on the login UI.
