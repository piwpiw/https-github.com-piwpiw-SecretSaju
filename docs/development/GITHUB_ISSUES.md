...
- [x] `src/lib/auth-mcp.ts` 및 `src/app/api/auth/mcp/callback/route.ts` 보안/중복 처리 가드 보강.
...
- `src/lib/notion.ts`: `insertNotionRow(data)` 구현/강화.
...
### Implementation Spec
- `src/lib/mail.ts`: `sendWelcomeEmail()`, `sendResultEmail()` 처리 구현 반영.
