...
- 에러 로그 → ERROR_LEDGER.md 기록 (세션 간 에러 기억 공유)
- 코드 패턴 에러 → docs/ERROR_CATALOG.md 등재 (재발 원천 차단)
...
```
Project Root:
├── AI_BOOTSTRAP.md       ← AI 온보딩 진입점 (Always First Read)
├── ERROR_LEDGER.md       ← 에러 이력 & 할루시네이션 방지
...
├── MASTER_PRD.md         ← 비즈니스 요구사항
├── ERROR_CATALOG.md      ← 코드 레벨 에러 패턴 (↔ ERROR_LEDGER 연동)
└── 00-overview/DEEP_HISTORY.md ← 영구 결정 기록
```
